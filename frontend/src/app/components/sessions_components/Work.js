        ...
        <div id={"tooltip_" + id} onClick={() => navigate('seance/' + id)} className={style.Work} >
            ...
            <div style={{borderLeftColor:color}} className={style.WorkContent}>
                <Row style={{marginLeft: "7px"}}>
                    <div style={finished ?{textDecoration:"line-through", color:"rgb(211,211,211)"} : {} } className={style.WorkTitle}>{name} <Badge theme="success" style={{fontSize: "9px", display: "inline"}}>{type && type}</Badge></div>
                </Row>
                {duration?(
